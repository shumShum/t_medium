defmodule NodeTwo.RabbitService.Mock do
  use GenServer
  ...
  def handle_cast({:receive, %{text: text, date: date} = msg}, state) do
    NodeTwo.Message.create(text, date)
    if state.tester, do: send(state.tester, :received)
